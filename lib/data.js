...
async function notifyFamily(familyId, user, title, body, url = "/") {
  try {
    ...
    await fetch("/api/notify", {
      ...
      body: JSON.stringify({ familyId, title, body, url }),
    });
  } catch (e) {
...
export async function addListItem(familyId, list, item, user) {
  ...
  const who = item.assignedTo ? ` (till ${item.assignedTo})` : "";
  notifyFamily(familyId, user, `📋 ${list.name}`, `${firstName(user)} lade till "${item.text}"${who}`, "/");
}
...
export async function createEvent(familyId, event, user) {
  ...
  notifyFamily(familyId, user, `📅 Ny händelse`, `${firstName(user)} lade till "${event.title}"${when ? ` ${when}` : ""}`, "/");
}
...
export async function sendMessage(familyId, user, text) {
  ...
  notifyFamily(familyId, user, `💬 ${firstName(user)}`, trimmed, "/");
}
